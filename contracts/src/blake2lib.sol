...
library BLAKE2b {
    ...
    bytes32 private constant IS0 =
        bytes32(
            hex"08c9bdf267e6096a3ba7ca8485ae67bb2bf894fe72f36e3cf1361d5f3af54fa5"
        );
    bytes32 private constant IS1 =
        bytes32(
    ...
    function hash(
        ...
        while (readInputOffset + BLOCK_SIZE_BYTES <= input.length) {
            if (bufferUsed == BLOCK_SIZE_BYTES) {
                ...
                bytes8[1] memory tt = [
                    bytes8(reverseByteOrder(uint64(bytesProcessed)))
                ];
            ...
            assembly {
                mcopy(
                    add(add(context, 100), bufferUsed),
                    add(input, add(32, readInputOffset)),
                    BLOCK_SIZE_BYTES
                )
            }
